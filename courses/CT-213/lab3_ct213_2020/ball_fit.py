...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        neighbors_list = [theta + np.array([cos(i * pi/4), sin(i * pi/4)]) * delta for i in range(num_neighbors)]
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        neighbor = theta + np.array([cos(random_angle), sin(random_angle)]) * delta
        return neighbor
    ...
    def schedule(i):
        ...
        current_temperature = temperature0 / (1 + beta * i ** 2)
