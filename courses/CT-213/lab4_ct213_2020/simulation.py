...
class RobotSprite:
    ...
    def __init__(self, sprite_params):
        ...
        self.body = DrawingUtils.rectangle_to_polygon(
            (-r, -d / 2.0 + wt / 2.0, r + o, d - wt))
        self.right_wheel = DrawingUtils.rectangle_to_polygon(
            (-r, -d / 2.0 - wt / 2.0, 2 * r, wt))
        self.left_wheel = DrawingUtils.rectangle_to_polygon(
            (-r, d / 2.0 - wt / 2.0, 2 * r, wt))
        self.line_sensor = DrawingUtils.rectangle_to_polygon(
            (o - st / 2.0, -sw / 2.0, st, sw))
    ...
    def transform(pose, polygon):
        ...
        for point in polygon:
            x = pose.position.x + \
                point[0] * cos(pose.rotation) - point[1] * sin(pose.rotation)
            y = pose.position.y + \
                point[0] * sin(pose.rotation) + point[1] * cos(pose.rotation)
    ...
    def draw(self, window, pose):
        ...
        :type pose: Pose.
        """
        DrawingUtils.draw_polygon_on_screen(window, RobotSprite.transform(
            pose, self.left_wheel), self.wheel_color, 0)
        DrawingUtils.draw_polygon_on_screen(window, RobotSprite.transform(
            pose, self.right_wheel), self.wheel_color, 0)
        DrawingUtils.draw_polygon_on_screen(
            window, RobotSprite.transform(pose, self.body), self.robot_color, 0)
        DrawingUtils.draw_polygon_on_screen(window, RobotSprite.transform(
            pose, self.line_sensor), self.robot_color, 0)
...
class Simulation:
    ...
    def reset(self, controller_params=None):
        ...
        self.line_follower.reset(
            Pose(start.x, start.y, 0.0), controller_params)
        self.point_list = []
    ...
    def update(self):
        ...
        self.point_list.append((m2pix(self.line_follower.pose.position.x), m2pix(
            self.line_follower.pose.position.y)))
    ...
    def draw(self, window):
        ...
        for sensor_position in sensor_positions:
            DrawingUtils.draw_circle_on_screen(
