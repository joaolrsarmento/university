...
def plot_results():
    ...
    plt.grid()
    plt.savefig('line_parameters_convergence.%s' %
...
def print_text():
    ...
    """
    text = font.render('Episode time: %.1f/%.1f' %
                       (episode_time, MAX_EPISODE_TIME), True, (0, 0, 0))
    window.blit(text, (round(0.1 * SCREEN_WIDTH), round(0.05 * SCREEN_HEIGHT)))
    text = font.render('Training iteration: ' +
                       str(training_iteration), True, (0, 0, 0))
    window.blit(text, (round(0.6 * SCREEN_WIDTH), round(0.05 * SCREEN_HEIGHT)))
    text = font.render('Accelerated factor: ' +
                       str(accelerated_factor) + 'x', True, (0, 0, 0))
    ...
    text = font.render('Accelerated mode? ' +
                       str(accelerated_mode), True, (0, 0, 0))
    window.blit(text, (round(0.6 * SCREEN_WIDTH), round(0.2 * SCREEN_HEIGHT)))
    text = font.render(
        'A: activate/deactivate accelerated mode', True, (0, 0, 0))
...
line_follower = LineFollower(Pose(
    0.5, 0.5, 45.0 * pi / 180.0), controller_params, robot_params, sensor_params)
...
# if the execution is in accelerated mode (faster than realtime)
